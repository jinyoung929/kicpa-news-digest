...
매일 오전 10시경(KST), news.kicpa.or.kr의 신규 기사를 수집해 OpenAI API로 요약하고 카테고리별로 정리해 보여주는 회계 뉴스 자동화 서비스입니다.

회계법인 지원을 준비하며 매일 확인하던 뉴스 모니터링을 자동화하기 위해 만들었습니다. 특히 빅4·한공회·ESG/AI 관련 기사는 자동으로 강조 표시해, 짧은 시간에 중요한 뉴스만 확인할 수 있도록 했습니다.

자동화 흐름: Vercel Cron이 매일 정해진 시각에 API를 호출하면, GitHub Actions가 신규 기사 선별 → 본문 추출 → 3문장 요약 → 날짜별 JSON 저장 → 커밋을 수행하고, Vercel이 이를 감지해 자동 재배포합니다. 상시 서버 없이 이벤트 기반으로 동작합니다

## 화면 이미지

<img width="1051" height="691" alt="스크린샷 2026-07-01 오후 8 22 11" src="https://github.com/user-attachments/assets/073b35fd-fe36-4c6f-a571-6c1f6d0fc721" />
